...
let clientKey;
//Сеансовые ключи — ключевая информация в криптографии, вырабатываемая между двумя пользователями, обычно для защиты канала связи.
//Обычно сеансовым ключом является общий секрет — информация, которая вырабатывается на основе секретного ключа одной стороны и открытого ключа другой стороны.
let clientPublicKey;

let sessionSecret;

axios.get('http://localhost:5000/').then(({data: {prime, publicKey}}) => {
  ...
  axios.post('http://localhost:5000/session', {publicKey: clientPublicKey}).then(({data}) => {

    console.log(sessionSecret.toString());

    axios.get('http://localhost:5000/resource').then(async ({data: {txt}}) => {

      //расшифровываем сервера сооьшение
    ...
    axios.get('http://localhost:5000/verified-resource').then(async ({data: {txt, ver}}) => {
